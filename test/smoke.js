...
function bashEvent({ toolCallId, content, fullOutputPath, isError = false }) {
  return {
    ...
    details: fullOutputPath
      ? { fullOutputPath, truncation: { truncated: true } }
      : undefined,
  };
}
...
async function main() {
  ...
  try {
    check(
      "agent dir redirected to temp (snapshots won't touch real ~/.pi)",
    ...
    await writeFile(
      ...
      JSON.stringify({ maxLines: 200, maxBytes: 5120 }),
    );
    ...
    const big = bigOutput();
    const res = await pi.fire(
      "tool_result",
      bashEvent({ toolCallId: "7f3a", content: big }),
      ctx,
    );
    ...
    check(
      "stub does NOT leak the buried needle",
      !stubText.includes(NEEDLE),
      "needle should be indexed, not shown",
    );
    ...
    check(
      "failed-command output is captured",
      !!resFoot && footStub.includes("exec:f00d"),
    );
    check(
      "stub drops Pi's 'Full output' footer",
      !footStub.includes("Full output:"),
    );
    check("stub drops Pi's temp path", !footStub.includes("pi-bash-deadbeef"));
    check(
      "stub keeps the real exit status",
      footStub.includes("Command exited with code 1"),
    );
    const footLines = Number(
      /full output \((\d+) lines/.exec(footStub)?.[1] ?? 0,
    );
    check(
      "stub reports the COMPLETE line count, not Pi's 40-line tail",
      footLines >= 1500,
      `${footLines} lines`,
    );
    const footHit = await pi._tools.recall.execute(
      "call-f",
      { query: NEEDLE, source: "exec:f00d" },
      undefined,
      undefined,
      ctx,
    );
    check(
      "recall reaches lines buried above Pi's kept tail",
      (footHit?.content?.[0]?.text ?? "").includes(NEEDLE),
    );

    // (b) small output below gate → passed through (no replacement).
    const small = "line a\nline b\nline c";
    const resSmall = await pi.fire(
      "tool_result",
      bashEvent({ toolCallId: "0001", content: small }),
      ctx,
    );
    ...
    const resErr = await pi.fire(
      "tool_result",
      bashEvent({ toolCallId: "e44d", content: bigOutput(), isError: true }),
      ctx,
    );
    const errStub = resErr?.content?.[0]?.text ?? "";
    check(
      "large error output is still captured",
      !!resErr && errStub.includes("exec:e44d"),
    );
    check(
      "error stub does NOT leak the buried needle",
      !errStub.includes(NEEDLE),
    );
    const errHit = await pi._tools.recall.execute(
      "call-e",
      { query: NEEDLE, source: "exec:e44d" },
      undefined,
      undefined,
      ctx,
    );
    check(
      "recall retrieves needle from a failed command",
      (errHit?.content?.[0]?.text ?? "").includes(NEEDLE),
    );
    ...
    check(
      "recall retrieves the buried needle",
      hitText.includes(NEEDLE),
      hitText.slice(0, 80),
    );
    check(
      "recall hit cites source + line",
      /exec:7f3a @ line \d+/.test(hitText),
    );
    check(
      "recall hit shows originating command",
      hitText.includes("run-build.sh"),
    );

    // (d) recall with no match.
    const miss = await recall.execute(
      "call-2",
      { query: "absolutelynosuchtoken" },
      undefined,
      undefined,
      ctx,
    );
    check(
      "recall reports no matches cleanly",
      (miss?.content?.[0]?.text ?? "").includes("no matches"),
    );
    ...
    const restored = ctx2._notifications.some(([, m]) =>
      m.includes("restored"),
    );
    check("session restored a prior snapshot", restored);

    const hit2 = await pi2._tools.recall.execute(
      "c",
      { query: NEEDLE, source: "exec:7f3a" },
      undefined,
      undefined,
    ...
    check(
      "recall finds the needle AFTER restore",
      (hit2?.content?.[0]?.text ?? "").includes(NEEDLE),
    );
    ...
    const resOff = await pi3.fire(
      "tool_result",
      bashEvent({ toolCallId: "abcd", content: bigOutput() }),
      ctx3,
    );
    ...
    console.log(
      failed
        ? "SMOKE: FAIL"
        : "SMOKE: PASS — capture → index → stub → recall → persist/restore all work",
    );
  } finally {
