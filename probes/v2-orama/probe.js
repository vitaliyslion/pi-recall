...
async function buildIndex() {
  ...
  const outA = makeOutput({
    lines: 3000,
    needleAt: 1234,
    needleText: `${NEEDLE} eviction policy mismatch here`,
  });
  ...
  const outB = makeOutput({
    lines: 800,
    needleAt: 400,
    needleText: `${NEEDLE} appears in OTHER capture`,
  });
  ...
  console.log(
    `  built: ${docsA.length} chunks (A) + ${docsB.length} chunks (B), bytesA=${Buffer.byteLength(outA)}`,
  );
  return { db, srcA, srcB };
}
...
async function main() {
  ...
  const rA = await search(db, {
    term: NEEDLE,
    where: { source: { eq: srcA } },
    limit: 3,
  });
  const topA = rA.hits[0];
  check("found a hit in A", rA.count > 0 && topA);
  check(
    "top hit carries the buried needle line",
    topA?.document.text.includes("eviction policy mismatch"),
    topA ? `startLine=${topA.document.startLine}` : "no hit",
  );
  check(
    "top hit is scoped to source A",
    topA?.document.source === srcA,
    topA?.document.source,
  );
  check(
    "every hit is from source A (filter holds)",
    rA.hits.every((h) => h.document.source === srcA),
  );

  console.log("3. scope filter narrows: same term, source B");
  const rB = await search(db, {
    term: NEEDLE,
    where: { source: { eq: srcB } },
    limit: 3,
  });
  check(
    "source B returns its OWN chunk, not A's",
    rB.hits.length > 0 && rB.hits.every((h) => h.document.source === srcB),
    `B hits=${rB.hits.length}`,
  );
  check(
    "B's needle text differs from A's",
    rB.hits[0]?.document.text.includes("OTHER capture"),
  );
  ...
  check(
    "doc count survived restore",
    (await count(db2)) === (await count(db)),
    `restored=${await count(db2)} original=${await count(db)}`,
  );

  console.log("6. BM25 retrieval AFTER restore (the real V2 question)");
  const rR = await search(db2, {
    term: NEEDLE,
    where: { source: { eq: srcA } },
    limit: 3,
  });
  check(
    "buried needle still retrieved post-restore",
    rR.hits[0]?.document.text.includes("eviction policy mismatch"),
    rR.hits[0] ? `startLine=${rR.hits[0].document.startLine}` : "no hit",
  );
  check(
    "post-restore scope filter still holds",
    rR.hits.every((h) => h.document.source === srcA),
  );
  ...
  console.log(
    failed
      ? "V2 PROBE: FAIL"
      : "V2 PROBE: PASS — round-trip + persist/restore + scoped BM25 all work",
  );
