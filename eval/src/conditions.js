...
export const RECALL_PROJECT_CONFIG = { maxLines: 200, maxBytes: 5120, persist: false };
...
export async function resolveCondition(condition, extPath) {
  ...
  if (condition === "C") {
    try {
      ...
      const abs = isAbsolute(extPath) || extPath.includes("://") ? extPath : resolve(process.cwd(), extPath);
      ...
      return { skip: `condition C skipped: pi-recall extension not loadable at ${extPath} (${e.message})` };
    }
  }
